...
function renderGames() {
    ...
    if (visibleGames.length === 0) {
        ...
        statusBox.textContent =
            "Brak premier pasujących do wybranego filtra.";
    ...
    visibleGames.forEach(game => {
        calendar.appendChild(createGameCard(game));
    });
...
async function loadUpcomingGames() {
    statusBox.style.display = "grid";
    statusBox.textContent =
        "Pobieranie nadchodzących premier…";
    ...
    try {
        const response = await fetch(
            `${RAWG_API_URL}?${params.toString()}`
        );

        if (!response.ok) {
            throw new Error(
                `RAWG API zwróciło kod ${response.status}`
            );
        }
        ...
        console.error(
            "Nie udało się pobrać premier:",
            error
        );

        statusBox.style.display = "grid";
        statusBox.textContent =
            "Nie udało się pobrać premier. Sprawdź połączenie, klucz API i uruchom stronę przez lokalny serwer.";
    }
}
...
filters.addEventListener("click", event => {
    ...
    document
        .querySelectorAll(".release-filter")
        .forEach(item => {
